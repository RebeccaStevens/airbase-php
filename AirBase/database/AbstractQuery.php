...
abstract class AbstractQuery {
  ...
  private function _buildSqlArray(array $data, $buildKeys=false, $buildValues=true, $connector=' = ', $separator=', ', $addGraveAccentsToKeys=true, $addGraveAccentsToValues=false) {
    ...
    if ($i_max === 0) {                 // no data given
      return $str;
    }
    $i=0;
    foreach ($data as $key=>$value) {
      if ($buildKeys) {
        if ($addGraveAccentsToKeys) {
          $str .= $this->getGraveAccent() . str_replace('.', $this->getGraveAccent() . '.' . $this->getGraveAccent(), $key) . $this->getGraveAccent();
        }
        else {
          $str .= $key;
        }
      ...
      if ($buildValues) {
        if ($addGraveAccentsToValues) {
          $str .= $this->getGraveAccent() . str_replace('.', $this->getGraveAccent() . '.' . $this->getGraveAccent(), $value) . $this->getGraveAccent();
        }
        else {
          $str .= $value;
        }
      ...
      if (++$i < $i_max) {
        $str .= $separator;			    // increment i and then if we haven't appended all the data item yet, append a comma
      }
    }
    return $str;
  }
   ...
   */
    ...
    protected function _buildSqlOrderBy(array $orderByFields=null, array $orderByOrder=null) {
      ...
      if ($i_max <= 0) {
        return '';						    		                    // no order by fields given - we are done here
      }
      ...
      for ($i=0; true;) {
        ...
        if($orderByOrderSet) {
          $orderBy .= ($orderByOrder[$i] ? ' ASC' : ' DESC');
        }
        else {
          $orderBy .= ' ASC';
        }
        if(++$i < $i_max) {
          $orderBy .= ', ';					        // increment i and then if we haven't appended all the order by fields yet, append a comma
        }
        else {
    ...
    protected function _buildSqlLimit($limitOffset, $limitAmount) {
      if (!isset($limitAmount)) {
        return '';				            // if no limit amount was set, we don't need to do anything here
      }
